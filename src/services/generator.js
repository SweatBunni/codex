...
function resolveGradleVersion(mcVersion, loaderType = 'fabric') {
  if (!mcVersion) return '8.8';
  const [major, minor] = mcVersion.split('.').map(Number);
  
  // Forge has stricter Gradle compatibility than Fabric
  if (loaderType === 'forge') {
    // ForgeGradle 6.0.17 (MC 1.21) needs Gradle 8.1, not 8.8
    // ForgeGradle 5.1.x (MC 1.20) needs Gradle 8.1
    if (minor >= 20) return '8.1';
    return '8.0.1';
  }
...
function getForgeGradleVersion(mcVersion) {
  if (!mcVersion) return '6.0.17';
  const [major, minor] = mcVersion.split('.').map(Number);
  // Forge/Gradle compatibility matrix
  // MC 26+ doesn't have Forge yet (use Neoforge instead)
  // MC 1.21.x: ForgeGradle 6.0.17+
  // MC 1.20.x: ForgeGradle 5.1.75+
  // MC 1.19.x: ForgeGradle 5.1.x
  if (major >= 26) return null; // No Forge for MC 26+
  if (minor >= 21) return '6.0.17';
  if (minor >= 20) return '5.1.75';
  if (minor >= 19) return '5.1.75';
  return '5.1.75';
}

function getForgeLoaderVersion(mcVersion) {
  if (!mcVersion) return '52.0.29';
  const [major, minor] = mcVersion.split('.').map(Number);
  // Forge loader version mapping for different MC versions
  if (minor >= 21) return '52.0.29';
  if (minor >= 20) return '49.1.3';
  if (minor >= 19) return '44.1.56';
  return '40.2.20';
}
...
function buildSystemPrompt(request, thinkingLevel) {
  ...
  const forgeGradleVer = loader === 'forge' ? getForgeGradleVersion(mcVersion) : '';
  const forgeLoaderVer = loader === 'forge' ? getForgeLoaderVersion(mcVersion) : '';
  const gradleVer = resolveGradleVersion(mcVersion, loader);
...
- Default mod should just log a message when initializing`;
    ...
    const forgeGradleVer = getForgeGradleVersion(mcVersion) || '6.0.17';
    const forgeLoaderVer = getForgeLoaderVersion(mcVersion);
    const gradleVer = resolveGradleVersion(mcVersion, 'forge');
    loaderInstructions = `
FORGE REQUIREMENTS (MC ${mcVersion}):
...
build.gradle:
plugins {
  id 'net.minecraftforge.gradle' version '${forgeGradleVer}'
  id 'maven-publish'
  id 'java'
}
...
minecraft {
  ...
  runs {
    client { }
    server { }
    data { args '--mod', 'examplemod', '--all', '--output', file('src/generated/resources'), '--existing', file('src/main/resources') }
  }
}
...
dependencies {
  minecraft 'net.minecraftforge:forge:${mcVersion}-${forgeLoaderVer}'
}

jar {
  manifest { attributes 'Main-Class': 'net.minecraftforge.userdev.FMLModType' }
}

CRITICAL: Use 'minecraft' block for ForgeGradle configuration, NOT direct dependencies`;
...
async function writeGradleWrapper(workDir, mcVersion) {
  ...
  // Determine loader type from files in workspace to get correct gradle version
  let loaderType = 'fabric';
  const buildGradlePath = path.join(workDir, 'build.gradle');
  if (await fs.pathExists(buildGradlePath)) {
    const buildContent = await fs.readFile(buildGradlePath, 'utf8');
    if (buildContent.includes('net.minecraftforge.gradle')) loaderType = 'forge';
    if (buildContent.includes('neoforge')) loaderType = 'neoforge';
  }
  const gradleVersion = resolveGradleVersion(mcVersion, loaderType);
